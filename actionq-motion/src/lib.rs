...
use std::{collections::HashMap, f32, ops::Deref, path::Path};
...
pub enum Widget {
    ...
    /// Horizontal line
    HLine { y: f32 },
    /// Arc from a starting angle to a destination angle, counter-clockwise
    Arc {
...
pub struct Metadata {
    ...
    /// Current audio track that should be played
    ...
    fn from_lua(value: LuaValue, _: &Lua) -> LuaResult<Self> {
        if let LuaValue::Table(t) = value {
            ...
            return Ok(match widget_type.as_str() {
                ...
                "arc" => {
                    let center: LuaVec2 = t.get("center")?;
                    Widget::Arc {
                        center: center.0,
                        radius: t.get("radius")?,
                        from: t.get("from")?,
...
impl FromLua for LuaVec2 {
    fn from_lua(value: LuaValue, _: &Lua) -> LuaResult<Self> {
        if let LuaValue::Table(t) = value {
            return Ok(LuaVec2(Vec2::new(t.get("x")?, t.get("y")?)));
        }
        unimplemented!()
    }
}
...
impl LuaExercise {
    ...
    fn create_lua_ctx() -> LuaResult<Lua> {
        ...
        // Anticlockwise angle between a vector and 1,0
        ctx.globals().set(
            "plane_angle",
            ctx.create_function(|_, a: LuaVec2| {
                let mut angle = (-a.0.y).atan2(a.0.x);
                if angle < 0.0 {
    ...
    fn convert_skeleton(&self, skeleton: &Skeleton) -> LuaSkeleton {
        skeleton
            .iter()
            .map(|(k, v): (&String, &Vec2)| (k.clone(), (*v).into()))
            .collect()
    }
    ...
    pub fn process(&mut self, skeleton: &Skeleton) -> LuaResult<(bool, Option<StateOutput>)> {
        ...
        if self
            .required_joints
            .iter()
            .any(|j| !skeleton.contains_key(j))
        {
            let missing: Vec<String> = self
                .required_joints
                .iter()
                .cloned()
                .filter(|j| !skeleton.contains_key(j))
                .collect();
